...
async function resolveDeepLinkThread(
  ...
  if (jobId) {
    ...
    const thread: MessageThread = {
      ...
      quoteId: null,
      jobId,
      counterpartId: job.providerUserId,
      counterpartName: "",
      messages: [],
    };
  ...
  if (!quote || quote.ownerId !== userId) return null;
  const thread: MessageThread = {
    key: quoteId!,
    quoteId,
    jobId: null,
    counterpartId: quote.providerUserId,
    counterpartName: "",
    messages: [],
  };
  await resolveCounterpartNames([thread]);
  ...
  return (
    <Card className="flex flex-col gap-4">
      ...
      <form action={sendMessage} className="flex items-end gap-2">
        ...
        <input type="hidden" name="returnPath" value="/owner/messages" />
